...
ビルドエラーとリンタエラーを解消した後、**セクション19「ソースコードレビュー計画」**に基づいてソースレビューを実施してください。

詳細なレビュー観点、対象ファイル、フェーズ別チェックリスト、レビュー実施コマンドについては、セクション19を参照してください。

> AI への指示例：
> 「実装が完了したので、ビルドエラーとリンタエラーをチェックしてください。
> エラーがなければ、セクション19のレビュー計画に基づいてレビューを実施してください。」
...
実装完了後、以下の順序でレビューを実施する：

```
1. ビルドエラーチェック（構文チェック）
2. リンタチェック（型ヒント、命名規則、未使用変数）
3. エラー修正
4. ソースレビュー実施（フェーズ別）
5. レビュー指摘対応
6. 最終確認
...
- `app/auth.py`
- `app/secrets.py`
- `app/providers/*.py`（APIキー処理部分）

**レビュー観点**:
- [ ] APIキーがログに出力されていないか
- [ ] Secret Managerの認証エラーハンドリングは適切か
- [ ] JWT/HMAC/IAPの署名検証ロジックにバイパス可能な脆弱性はないか
- [ ] エラーメッセージに機密情報が含まれていないか
- [ ] 上流APIのエラー詳細が隠蔽されているか（502 Bad Gateway）
- [ ] タイムスタンプ検証の許容範囲は適切か
- [ ] 入力検証が適切に行われているか
...
**レビュー観点**:
- [ ] プロダクト設定のロードとフォールバック処理は適切か
- [ ] プロバイダ選択ロジックに漏れはないか
- [ ] レート制限のアトミック性は保証されているか
- [ ] エラーハンドリングが一貫しているか
- [ ] HTTPステータスコードの使い分けは正しいか
- [ ] 環境変数の読み込みとデフォルト値は適切か
- [ ] CORSやセキュリティヘッダーの設定は適切か
...
**対象ファイル**:
- `app/providers/*.py`（レスポンス変換部分）
- `app/upstream.py`（タイムアウト設定）
- ログ出力全般

**レビュー観点**:
- [ ] 不要な同期処理がないか（async/awaitの適切な使用）
- [ ] N+1問題やメモリリークの可能性はないか
- [ ] タイムアウト設定は適切か
- [ ] ログレベルは適切か（INFO/WARNING/ERROR）
- [ ] 構造化ログ（extra）が活用されているか
- [ ] デバッグログがコミット前に削除されているか
- [ ] エラー時のスタックトレース出力は適切か
...
**レビュー観点**:
- [ ] 関数の単一責任原則が守られているか
- [ ] DRY原則（重複コード）が守られているか
- [ ] 変数名・関数名は意図が明確か
- [ ] docstringは適切に記述されているか
- [ ] 型ヒントは完全に記述されているか
- [ ] コメントは必要最低限か（自明なものを除く）
- [ ] マジックナンバーは定数化されているか
...
#### ⚠️ 指摘事項
- [app/auth.py:123] タイムスタンプ検証の許容範囲が広すぎる（300秒 → 60秒推奨）
- [app/providers/gemini.py:233] エラーメッセージに上流APIのレスポンステキストが含まれている
...
#### ⚠️ 指摘事項
- [app/config.py:99] マルチプロバイダ形式の判定ロジックが脆弱
...
```bash
# 構文チェック
python -m py_compile app/*.py app/providers/*.py
...
| 重大度 | アクション |
|--------|----------|
| Critical | 即座に修正してコミット前に再レビュー |
| High | 次回リリース前に修正 |
| Medium | バックログに追加、優先度中 |
| Low | ドキュメント化のみ、対応は任意 |

> AI への指示例：
> 「実装完了後、ビルドエラーとリンタチェックを実施し、
> その後フェーズ別にソースコードレビューを実施してください。
> レビュー結果は上記の形式で記録し、Critical/Highの指摘事項は即座に修正してください。」
...
- 2025-12-18: セクション19「ソースコードレビュー計画」を追加
  - フェーズ別レビュー観点（Critical/High/Medium/Low）
  - レビュー結果の記録形式
  - レビュー後のアクション基準
